...
"""
Application Test: AT1.9 - Session History with Qdrant Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for session history persistence and retrieval with Qdrant vector database
...
Recent Changes:
- Initial implementation for Qdrant vector database integration
"""
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_9_qdrant_{unique_id}",
        "email": build_test_email("at1_9_qdrant", unique_id, base_email),
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_9_qdrant_{unique_id}",
        "title": f"Test Expert {unique_id}",
        "description": "Test expert for Qdrant history",
...
def qdrant_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "qdrant", "history")
    try:
        yield store
    finally:
...
def test_session_history_qdrant_persistence(
    api_client, test_user, test_expert, qdrant_vector_store
):
    """Test that session messages can be stored in Qdrant vector database.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - qdrant_vector_store: Qdrant vector store created via API
    - messages: Array of messages to store

    Expected Outputs:
    - Session created successfully
    - Messages added to session via API
    - Messages stored in Qdrant vector store
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Qdrant History Test {unique_id}",
    ...
    messages_to_add = [
        {"role": "user", "content": f"User message 1 for Qdrant test {unique_id}"},
        {"role": "assistant", "content": f"Assistant response 1 for Qdrant test {unique_id}"},
        {"role": "user", "content": f"User message 2 for Qdrant test {unique_id}"},
    ]
    ...
    # Store messages in Qdrant vector store
    stored_doc_ids = []
    ...
    # Verify messages are in Qdrant
    assert len(stored_doc_ids) == len(messages_to_add), (
        f"Expected {len(messages_to_add)} documents in Qdrant, got {len(stored_doc_ids)}"
    )

    # Query Qdrant to verify documents
    ...
    assert any(r.get("document") == messages_to_add[0]["content"] for r in results), (
        "Expected stored message not found in Qdrant results"
    )
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")
...
def test_session_history_qdrant_retrieval(api_client, test_user, test_expert, qdrant_vector_store):
    """Test retrieving session history from Qdrant vector database.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - qdrant_vector_store: Qdrant vector store
    - messages: Array of messages stored in Qdrant
    ...
    assert "results" in query_result, "Query result missing results"
    results = query_result.get("results") or []
    assert len(results) > 0, "No results returned from Qdrant query"
    assert any(r.get("document") == messages_content[0] for r in results), (
        "Expected message not found in Qdrant results"
    )
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")
...
def test_session_history_qdrant_semantic_search(
    api_client, test_user, test_expert, qdrant_vector_store
):
    """Test semantic search in Qdrant vector database for session history.
...
def test_session_history_qdrant_collection_isolation(
    api_client, test_user, test_expert, qdrant_vector_store
):
    """Test that Qdrant collections provide isolation between sessions.
...
def test_session_history_qdrant_pagination(api_client, test_user, test_expert, qdrant_vector_store):
    """Test pagination when retrieving session history from Qdrant.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - qdrant_vector_store: Qdrant vector store
    - Many messages stored in Qdrant
...
def test_session_history_qdrant_message_order(
    api_client, test_user, test_expert, qdrant_vector_store
):
    """Test that message order is maintained when retrieving from Qdrant.
    ...
    Expected Outputs:
    - Messages stored in Qdrant with metadata
    ...
    # Store messages in Qdrant with timestamps
    stored_timestamps = []
    for i, msg in enumerate(messages):
