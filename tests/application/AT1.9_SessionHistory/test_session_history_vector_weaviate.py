...
"""
Application Test: AT1.9 - Session History with Weaviate Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for session history persistence and retrieval with Weaviate vector database
...
Recent Changes:
- Initial implementation for Weaviate vector database integration
"""
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_9_weaviate_{unique_id}",
        "email": f"at1_9_weaviate_{unique_id}@{domain}",
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_9_weaviate_{unique_id}",
        "title": f"Test Expert {unique_id}",
        "description": "Test expert for Weaviate history",
...
@pytest.fixture
def weaviate_vector_store(api_client):
    """Create Weaviate vector store for testing."""
    store = create_test_vector_store(api_client, "weaviate", "history")
    try:
        yield store
    finally:
...
def test_session_history_weaviate_persistence(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test that session messages can be stored in Weaviate vector database.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - weaviate_vector_store: Weaviate vector store created via API
    - messages: Array of messages to store

    Expected Outputs:
    - Session created successfully
    - Messages added to session via API
    - Messages stored in Weaviate vector store
    - embedding_id set in SQL messages (if supported)
    """
    user_id = test_user["id"]
    expert_id = test_expert["id"]
    vector_store_id = weaviate_vector_store["id"]
    collection = weaviate_vector_store.get("config", {}).get("collection_name")
    assert collection, "Weaviate vector store config missing collection_name"
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Weaviate History Test {unique_id}",
    ...
    messages_to_add = [
        {"role": "user", "content": f"User message 1 for Weaviate test {unique_id}"},
        {"role": "assistant", "content": f"Assistant response 1 for Weaviate test {unique_id}"},
        {"role": "user", "content": f"User message 2 for Weaviate test {unique_id}"},
    ]
    ...
    # Store messages in Weaviate vector store
    stored_doc_ids = []
    for i, msg in enumerate(messages_to_add):
        ...
        stored_doc_ids.append(doc_response.get("id") or doc_id)

    # Verify messages are in Weaviate
    assert len(stored_doc_ids) == len(messages_to_add), (
        f"Expected {len(messages_to_add)} documents in Weaviate, got {len(stored_doc_ids)}"
    )

    # Query Weaviate to verify documents
    ...
    assert any(r.get("document") == messages_to_add[0]["content"] for r in results), (
        "Expected stored message not found in Weaviate results"
    )
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")
...
def test_session_history_weaviate_retrieval(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test retrieving session history from Weaviate vector database.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - weaviate_vector_store: Weaviate vector store
    - messages: Array of messages stored in Weaviate

    Expected Outputs:
    - Messages stored in Weaviate
    ...
    assert "results" in query_result, "Query result missing results"
    results = query_result.get("results") or []
    assert len(results) > 0, "No results returned from Weaviate query"
    assert any(r.get("document") == messages_content[0] for r in results), (
        "Expected message not found in Weaviate results"
    )
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")
...
def test_session_history_weaviate_semantic_search(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test semantic search in Weaviate vector database for session history.
...
def test_session_history_weaviate_collection_isolation(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test that Weaviate collections provide isolation between sessions.
...
def test_session_history_weaviate_pagination(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test pagination when retrieving session history from Weaviate.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - weaviate_vector_store: Weaviate vector store
    - Many messages stored in Weaviate
...
def test_session_history_weaviate_message_order(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test that message order is maintained when retrieving from Weaviate.
    ...
    Expected Outputs:
    - Messages stored in Weaviate with metadata
    ...
    # Store messages in Weaviate with timestamps
    stored_timestamps = []
    for i, msg in enumerate(messages):
