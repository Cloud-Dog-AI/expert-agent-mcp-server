...
"""
Application Test: AT1.9 - Session History with Chroma Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for session history persistence and retrieval with Chroma vector database
...
Recent Changes:
- Initial implementation for Chroma vector database integration
"""
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_9_chroma_{unique_id}",
        "email": build_test_email("at1_9_chroma", unique_id, base_email),
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_9_chroma_{unique_id}",
        "title": f"Chroma Session History Retrieval Expert {unique_id}",
        "description": (
            "Expert profile for chroma session history validation with "
            "pagination retrieval semantic context and isolation checks."
        ),
...
def chroma_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "chroma", "history")
    try:
        yield store
    finally:
...
def test_session_history_chroma_persistence(
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Chroma History Test {unique_id}",
    ...
    messages_to_add = [
        {"role": "user", "content": f"User message 1 for Chroma test {unique_id}"},
        {"role": "assistant", "content": f"Assistant response 1 for Chroma test {unique_id}"},
        {"role": "user", "content": f"User message 2 for Chroma test {unique_id}"},
    ]
    ...
    # Store messages in Chroma vector store
    stored_doc_ids = []
    ...
    # Verify messages are in Chroma
    assert len(stored_doc_ids) == len(messages_to_add), (
        f"Expected {len(messages_to_add)} documents in Chroma, got {len(stored_doc_ids)}"
    )

    # Query Chroma to verify documents
    ...
    assert "results" in query_result or "documents" in query_result, (
        "Query result missing results/documents"
    )
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")
...
def test_session_history_chroma_retrieval(api_client, test_user, test_expert, chroma_vector_store):
    """Test retrieving session history from Chroma vector database.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - chroma_vector_store: Chroma vector store
    - messages: Array of messages stored in Chroma
    ...
    results = query_result.get("results") or query_result.get("documents") or []
    assert len(results) > 0, "No results returned from Chroma query"
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")


def test_session_history_chroma_semantic_search(
    api_client, test_user, test_expert, chroma_vector_store
):
    """Test semantic search in Chroma vector database for session history.
    ...
    result_contents = [r.get("document") or r.get("content") or "" for r in results]
    python_found = any(
...
def test_session_history_chroma_collection_isolation(
    api_client, test_user, test_expert, chroma_vector_store
):
    """Test that Chroma collections provide isolation between sessions.
    ...
    unique_id = str(uuid.uuid4())[:8]
    collection1 = f"session1_{unique_id}"
    collection2 = f"session2_{unique_id}"
    ...
    # Store messages in different collections
    add_message_to_vector_store(
        api_client,
        vector_store_id,
        collection1,
        f"Session 1 message {unique_id}",
    ...
    results1 = query_result1.get("results") or query_result1.get("documents") or []
    results2 = query_result2.get("results") or query_result2.get("documents") or []
    result1_contents = [r.get("document") or r.get("content") or "" for r in results1]
    result2_contents = [r.get("document") or r.get("content") or "" for r in results2]
    session2_in_collection1 = any("Session 2" in content for content in result1_contents)
    assert not session2_in_collection1, (
        "Collection isolation failed: Session 2 message found in collection1"
    )
    session1_in_collection2 = any("Session 1" in content for content in result2_contents)
    assert not session1_in_collection2, (
        "Collection isolation failed: Session 1 message found in collection2"
    )
@pytest.mark.AT
@pytest.mark.mcp
@pytest.mark.req("FR-013")
...
def test_session_history_chroma_pagination(api_client, test_user, test_expert, chroma_vector_store):
    """Test pagination when retrieving session history from Chroma.

    Inputs:
    - test_user: User created via API
    - test_expert: Expert created via API
    - chroma_vector_store: Chroma vector store
    - Many messages stored in Chroma
...
def test_session_history_chroma_message_order(
    api_client, test_user, test_expert, chroma_vector_store
):
    """Test that message order is maintained when retrieving from Chroma.
    ...
    Expected Outputs:
    - Messages stored in Chroma with metadata
    ...
    # Store messages in Chroma with timestamps
    stored_timestamps = []
    for i, msg in enumerate(messages):
