...
"""
Application Test: AT1.10 - Context Window Management with Weaviate Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for context window management with Weaviate vector database integration
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_10_weaviate_{unique_id}",
        "email": f"at1_10_weaviate_{unique_id}@{domain}",
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_10_weaviate_{unique_id}",
        "title": f"Weaviate Context Window Expert {unique_id} schema retrieval embeddings semantic search metadata",
        "description": "Weaviate-backed context window scenario with unique vocabulary: alpine beacon cipher delta ember fjord glyph helios isotope juniper keystone",
...
def weaviate_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "weaviate", "context_window")
    try:
        yield store
    finally:
...
def test_context_window_management_with_weaviate_persistence(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test context window management when messages are stored in Weaviate.
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Weaviate Context Window Test {unique_id}",
...
def test_context_window_summarization_with_weaviate_via_api(
    api_client, test_user, test_expert, weaviate_vector_store
):
...
def test_context_window_truncation_with_weaviate_via_api(
    api_client, test_user, test_expert, weaviate_vector_store
):
...
def test_context_window_semantic_search_with_weaviate_via_api(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test context window management with semantic search in Weaviate.
