...
"""
Application Test: AT1.10 - Context Window Management with Opensearch Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for context window management with Opensearch vector database integration
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_10_opensearch_{unique_id}",
        "email": build_test_email("at1_10_opensearch", unique_id, base_email),
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_10_opensearch_{unique_id}",
        "title": f"OpenSearch Context Window Expert {unique_id} indexing retrieval embeddings semantic search metadata",
        "description": "OpenSearch-backed context window scenario with unique vocabulary: alpine beacon cipher delta ember fjord glyph helios isotope juniper keystone",
...
def opensearch_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "opensearch", "context_window")
    try:
        yield store
    finally:
...
def test_context_window_management_with_opensearch_persistence(
    api_client, test_user, test_expert, opensearch_vector_store
):
    """Test context window management when messages are stored in Opensearch.
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Opensearch Context Window Test {unique_id}",
...
def test_context_window_summarization_with_opensearch_via_api(
    api_client, test_user, test_expert, opensearch_vector_store
):
...
def test_context_window_truncation_with_opensearch_via_api(
    api_client, test_user, test_expert, opensearch_vector_store
):
...
def test_context_window_semantic_search_with_opensearch_via_api(
    api_client, test_user, test_expert, opensearch_vector_store
):
    """Test context window management with semantic search in Opensearch.
