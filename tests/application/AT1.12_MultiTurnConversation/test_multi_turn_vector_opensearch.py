...
"""
Application Test: AT1.12 - Multi-turn Conversation with Context Retention via Opensearch Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for multi-turn conversation with context retention using Opensearch vector database
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_12_opensearch_{unique_id}",
        "email": build_test_email("at1_12_opensearch", unique_id, base_email),
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_12_opensearch_{unique_id}",
        "title": f"OpenSearch Multi Turn Expert {unique_id} indexing retrieval embeddings semantic search metadata",
        "description": "OpenSearch multi-turn scenario with unique vocabulary: acorn basil cipher dune ember fjord glyph helios isotope juniper keystone",
...
def opensearch_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "opensearch", "multi_turn")
    try:
        yield store
    finally:
...
def test_multi_turn_management_with_opensearch_persistence(
    api_client, test_user, test_expert, opensearch_vector_store
):
    """Test multi-turn management when messages are stored in Opensearch.
    ...
    - Messages stored in both SQL and Opensearch
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Opensearch Multi-turn Test {unique_id}",
    ...
    for i in range(message_count):
        message_content = f"Message {i} for Opensearch context test {unique_id}: " + "P" * 40
    ...
    # Verify messages in Opensearch
    query_result = query_vector_store(
        api_client,
        vector_store_id,
        collection,
        f"Message 0 for OpenSearch context test {unique_id}",
        n_results=5,
    )
...
def test_multi_turn_summarization_with_opensearch_via_api(
    api_client, test_user, test_expert, opensearch_vector_store
):
    """Test summarization when messages are stored in Opensearch.
    ...
    - Summarization works with Opensearch-backed messages
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Opensearch Summarization Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    # Add messages and store in Opensearch
    message_count = int(get_config("test.at1_12.message_counts.xlong"))
    for i in range(message_count):
        message_content = f"Message {i} for Opensearch summary test {unique_id}: " + "Q" * 45
        msg_response = api_client.post(
            f"/sessions/{session_id}/messages", json={"role": "user", "content": message_content}
        )
        assert msg_response.status_code == 200, f"Failed to add message {i}: {msg_response.text}"
    ...
    try:
        ...
        pytest.fail(f"Summarization request exceeded HTTP timeout in this environment: {e}")
...
def test_multi_turn_truncation_with_opensearch_via_api(
    api_client, test_user, test_expert, opensearch_vector_store
):
    """Test truncation when messages are stored in Opensearch.
    ...
    Expected Outputs:
    - Messages stored in Opensearch
    - Truncation works with Opensearch-backed messages
    - Recent messages preserved
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Opensearch Truncation Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    for i in range(message_count):
        message_content = f"Message {i} for Opensearch truncation test {unique_id}: " + "R" * 40
        msg_response = api_client.post(
            f"/sessions/{session_id}/messages", json={"role": "user", "content": message_content}
        )
        assert msg_response.status_code == 200, f"Failed to add message {i}: {msg_response.text}"
...
def test_multi_turn_semantic_search_with_opensearch_via_api(
    api_client, test_user, test_expert, opensearch_vector_store
):
    """Test multi-turn management with semantic search in Opensearch.
    ...
    - semantic search: Query Opensearch for relevant context
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Opensearch Semantic Search Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
