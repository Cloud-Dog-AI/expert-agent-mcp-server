...
"""
Application Test: AT1.12 - Multi-turn Conversation with Context Retention via Weaviate Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for multi-turn conversation with context retention using Weaviate vector database
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_12_weaviate_{unique_id}",
        "email": f"at1_12_weaviate_{unique_id}@{domain}",
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_12_weaviate_{unique_id}",
        "title": f"Weaviate Multi Turn Expert {unique_id} schema retrieval embeddings semantic search metadata",
        "description": "Weaviate multi-turn scenario with unique vocabulary: acorn basil cipher dune ember fjord glyph helios isotope juniper keystone",
...
def weaviate_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "weaviate", "multi_turn")
    try:
        yield store
    finally:
...
def test_multi_turn_management_with_weaviate_persistence(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test multi-turn management when messages are stored in Weaviate.
    ...
    - Messages stored in both SQL and Weaviate
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Weaviate Multi-turn Test {unique_id}",
    ...
    for i in range(message_count):
        message_content = f"Message {i} for Weaviate context test {unique_id}: " + "P" * 40
    ...
    # Verify messages in Weaviate
    query_result = query_vector_store(
        api_client,
        vector_store_id,
        collection,
        f"Message 0 for Weaviate context test {unique_id}",
        n_results=5,
    )
...
def test_multi_turn_summarization_with_weaviate_via_api(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test summarization when messages are stored in Weaviate.
    ...
    - Summarization works with Weaviate-backed messages
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Weaviate Summarization Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    # Add messages and store in Weaviate
    message_count = int(get_config("test.at1_12.message_counts.xlong"))
    for i in range(message_count):
        message_content = f"Message {i} for Weaviate summary test {unique_id}: " + "Q" * 45
        msg_response = api_client.post(
            f"/sessions/{session_id}/messages", json={"role": "user", "content": message_content}
        )
        assert msg_response.status_code == 200, f"Failed to add message {i}: {msg_response.text}"
...
def test_multi_turn_truncation_with_weaviate_via_api(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test truncation when messages are stored in Weaviate.
    ...
    Expected Outputs:
    - Messages stored in Weaviate
    - Truncation works with Weaviate-backed messages
    - Recent messages preserved
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Weaviate Truncation Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    for i in range(message_count):
        message_content = f"Message {i} for Weaviate truncation test {unique_id}: " + "R" * 40
        msg_response = api_client.post(
            f"/sessions/{session_id}/messages", json={"role": "user", "content": message_content}
        )
        assert msg_response.status_code == 200, f"Failed to add message {i}: {msg_response.text}"
...
def test_multi_turn_semantic_search_with_weaviate_via_api(
    api_client, test_user, test_expert, weaviate_vector_store
):
    """Test multi-turn management with semantic search in Weaviate.
    ...
    - semantic search: Query Weaviate for relevant context
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Weaviate Semantic Search Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
