...
"""
Application Test: AT1.12 - Multi-turn Conversation with Context Retention via Qdrant Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for multi-turn conversation with context retention using Qdrant vector database
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_12_qdrant_{unique_id}",
        "email": build_test_email("at1_12_qdrant", unique_id, base_email),
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_12_qdrant_{unique_id}",
        "title": f"Qdrant Multi Turn Expert {unique_id} vectors payload retrieval embeddings semantic memory metadata",
        "description": "Qdrant multi-turn scenario with unique vocabulary: acorn basil cipher dune ember fjord glyph helios isotope juniper keystone",
...
def qdrant_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "qdrant", "multi_turn")
    try:
        yield store
    finally:
...
def test_multi_turn_management_with_qdrant_persistence(
    api_client, test_user, test_expert, qdrant_vector_store
):
    """Test multi-turn management when messages are stored in Qdrant.
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Qdrant Multi-turn Test {unique_id}",
    ...
    for i in range(message_count):
        ...
        # Store in Qdrant (requires UUID IDs)
        add_message_to_vector_store(
            api_client,
            vector_store_id,
            collection,
...
def test_multi_turn_summarization_with_qdrant_via_api(
    api_client, test_user, test_expert, qdrant_vector_store
):
...
def test_multi_turn_truncation_with_qdrant_via_api(
    api_client, test_user, test_expert, qdrant_vector_store
):
...
def test_multi_turn_semantic_search_with_qdrant_via_api(
    api_client, test_user, test_expert, qdrant_vector_store
):
    """Test multi-turn management with semantic search in Qdrant.
