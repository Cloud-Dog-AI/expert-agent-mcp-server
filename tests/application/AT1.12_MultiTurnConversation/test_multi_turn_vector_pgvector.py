...
"""
Application Test: AT1.12 - Multi-turn Conversation with Context Retention via Pgvector Vector Database

License: Apache 2.0
Ownership: Cloud Dog
Description: Tests for multi-turn conversation with context retention using Pgvector vector database
...
def test_user_credentials(test_env_file, test_secrets_file):
    ...
    return {
        "username": f"{base_username}_at1_12_pgvector_{unique_id}",
        "email": build_test_email("at1_12_pgvector", unique_id, base_email),
...
def test_expert(api_client, test_config):
    ...
    unique_id = str(uuid.uuid4())[:8]
    expert_data = {
        "name": f"expert_at1_12_pgvector_{unique_id}",
        "title": f"PGVector Multi Turn Expert {unique_id} persistence retrieval embeddings semantic search metadata",
        "description": "PGVector multi-turn scenario with unique vocabulary: acorn basil cipher dune ember fjord glyph helios isotope juniper keystone",
...
def pgvector_vector_store(api_client):
    ...
    store = create_test_vector_store(api_client, "pgvector", "multi_turn")
    try:
        yield store
    finally:
...
def test_multi_turn_management_with_pgvector_persistence(
    api_client, test_user, test_expert, pgvector_vector_store
):
    """Test multi-turn management when messages are stored in Pgvector.
    ...
    - Messages stored in both SQL and Pgvector
    ...
    session_response = api_client.post(
        ...
        json={
            ...
            "title": f"Pgvector Multi-turn Test {unique_id}",
    ...
    for i in range(message_count):
        message_content = f"Message {i} for Pgvector context test {unique_id}: " + "P" * 40
    ...
    # Verify messages in Pgvector
    query_result = query_vector_store(
        api_client,
        vector_store_id,
        collection,
        f"Message 0 for PGVector context test {unique_id}",
        n_results=5,
    )
...
def test_multi_turn_summarization_with_pgvector_via_api(
    api_client, test_user, test_expert, pgvector_vector_store
):
    """Test summarization when messages are stored in Pgvector.
    ...
    - Summarization works with Pgvector-backed messages
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Pgvector Summarization Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    # Add messages and store in Pgvector
    message_count = int(get_config("test.at1_12.message_counts.xlong"))
    for i in range(message_count):
        message_content = f"Message {i} for Pgvector summary test {unique_id}: " + "Q" * 45
        msg_response = api_client.post(
            f"/sessions/{session_id}/messages", json={"role": "user", "content": message_content}
        )
        assert msg_response.status_code == 200, f"Failed to add message {i}: {msg_response.text}"
...
def test_multi_turn_truncation_with_pgvector_via_api(
    api_client, test_user, test_expert, pgvector_vector_store
):
    """Test truncation when messages are stored in Pgvector.
    ...
    Expected Outputs:
    - Messages stored in Pgvector
    - Truncation works with Pgvector-backed messages
    - Recent messages preserved
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Pgvector Truncation Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    for i in range(message_count):
        message_content = f"Message {i} for Pgvector truncation test {unique_id}: " + "R" * 40
        msg_response = api_client.post(
            f"/sessions/{session_id}/messages", json={"role": "user", "content": message_content}
        )
        assert msg_response.status_code == 200, f"Failed to add message {i}: {msg_response.text}"
...
def test_multi_turn_semantic_search_with_pgvector_via_api(
    api_client, test_user, test_expert, pgvector_vector_store
):
    """Test multi-turn management with semantic search in Pgvector.
    ...
    - semantic search: Query Pgvector for relevant context
    ...
    session_response = api_client.post(
        ...
        json={
            "user_id": user_id,
            "expert_config_id": expert_id,
            "title": f"Pgvector Semantic Search Test {unique_id}",
            "context_window": context_window,
        },
    )
    assert session_response.status_code == 200, f"Failed to create session: {session_response.text}"
    ...
    # PGVector indexing/visibility can be slightly delayed on some local setups.
    # Retry briefly before concluding semantic retrieval failed.
    for _ in range(5):
        if "results" in query_result and (query_result.get("results") or []):
            break
        time.sleep(0.5)
        query_result = query_vector_store(
            api_client,
