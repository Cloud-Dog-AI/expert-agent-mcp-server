...
def test_at1103_knowledge_history_crud_permission_access(test_env_file):
    # Covers: FR1.31
    ...
    try:
        ...
        assert int(versions_payload.get("count", 0)) >= 1
        version_items = versions_payload.get("versions", [])
        assert version_items
        rollback_version = int(version_items[0]["version"])

        rollback = s1.post(
            f"{base}/knowledge/user/{user_id_1}/rollback",
            json={"version": rollback_version},
            timeout=timeout,
        )
        assert rollback.status_code == 200, rollback.text
        assert rollback.json().get("success") is True
