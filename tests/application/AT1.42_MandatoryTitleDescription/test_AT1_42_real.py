...
License: Apache 2.0
Ownership: Cloud Dog
Description: REAL Application Test AT1.42 - Mandatory title and description for experts.
**************************************************
"""
...
def test_AT1_42_requires_title_and_description(request: pytest.FixtureRequest):
    env_file = request.config.getoption("--env")
    if not env_file:
        pytest.fail("--env is REQUIRED")
    store = TestOutputStorage(
        "AT1.42_MandatoryTitleDescription", "test_AT1_42_requires_title_and_description"
    )
    ...
    # Missing description should be rejected (422 from validation)
    resp = client.post(
        "/experts",
        json={
            "name": f"at142_{suffix}",
            "title": "AT1.42",
    ...
    store.save_operation(
        "create_expert_missing_description",
        {},
        {"status_code": resp.status_code, "body": resp.text},
    )
    store.save_validation(
        "missing_description_422",
