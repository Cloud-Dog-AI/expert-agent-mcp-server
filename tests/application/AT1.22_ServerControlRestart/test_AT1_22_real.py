...
License: Apache 2.0
Ownership: Cloud Dog
Description: REAL Application Test AT1.22 - server_control.sh restart command.

Validates `./server_control.sh restart api` using ONLY the approved server control script
and REAL HTTP checks.
...
def test_AT1_22_restart_api_server(request: pytest.FixtureRequest):
    """
    AT1.22: Restart API server.

    Validations:
    - restart command exit code 0
    - /health returns 200 after restart
    """
    env_file = _require_env_file(request)
    store = TestOutputStorage("AT1.22_ServerControlRestart", "test_AT1_22_restart_api_server")
    ...
    for _ in range(30):
        try:
            ...
            pass
        time.sleep(1)
    store.save_validation("health_200_after_restart", {"ok": ok}, passed=ok)
