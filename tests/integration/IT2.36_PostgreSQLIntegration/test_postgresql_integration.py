...
"""
Integration Test: IT2.36 - PostgreSQL integration (schema + CRUD + migrations)
"""
...
def test_it236_postgresql_uri_and_connectivity(test_env_file):
    load_config.cache_clear()
    uri = str(get_config("db.uri") or "")
    if not uri.startswith("postgresql://"):
        pytest.fail(f"Expected postgresql:// db.uri for PostgreSQL env, got: {uri}")
...
def test_it236_postgresql_schema_and_crud(test_env_file):
    init_db(force_reinit=True)
    engine = get_engine()
    ...
    try:
        ...
        user = manager.create_user(
            username=f"it236_{suffix}",
            email=f"it236_{suffix}@example.com",
            password=str(password),
        )
        fetched = manager.get_user(user_id=user.id)
        assert fetched is not None
        assert fetched.username == f"it236_{suffix}"
        db.delete(user)
        db.commit()
...
def test_it236_postgresql_migration_assets_present(test_env_file):
