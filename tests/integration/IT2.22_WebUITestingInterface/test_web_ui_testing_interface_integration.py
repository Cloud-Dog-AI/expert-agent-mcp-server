...
def ui_entities(clients):
    ...
    user = api.post(
        f"{api_url}/users",
        json={
            "username": f"{base_username}_it222_{u}",
            "email": f"it222_{u}@{domain}",
            "password": str(password),
            "display_name": f"IT222 {u}",
        },
    ...
    expert = api.post(
        f"{api_url}/experts",
        json={
            "name": f"it222_expert_{u}",
            "title": f"IT222 Expert {u}",
            "description": f"IT2.22 testing-interface integration expert token {u}.",
    ...
    channel = api.post(
        f"{api_url}/channels",
        json={
            "name": f"IT222 Channel {u}",
            "expert_config_id": int(expert_data["id"]),
            "description": f"UI testing integration {u}",
            "enabled": True,
        },
        timeout=api.timeout_seconds,
    )
...
def test_it222_web_ui_testing_panel_calls_testing_api(clients, ui_entities):
    ...
    try:
        ...
        browser.get(f"{clients['web_url']}/testing")
        wait.until(EC.visibility_of_element_located((By.ID, "testChannelId")))
        channel_id = str(int(ui_entities["channel"]["id"]))
        channel_input = browser.find_element(By.ID, "testChannelId")
        channel_input.clear()
        channel_input.send_keys(channel_id)
        wait.until(lambda d: d.find_element(By.ID, "testChannelId").get_attribute("value") == channel_id)
        browser.find_element(By.ID, "runTestSuiteButton").click()

        WebDriverWait(browser, 120).until(
            lambda d: len((d.find_element(By.ID, "testResults").text or "").strip()) > 0
        )

        text = browser.find_element(By.ID, "testResults").text.strip()
        if text.lower().startswith("error:"):
            pytest.fail(f"testing panel returned error: {text}")
        parsed = json.loads(text)
        assert isinstance(parsed, dict), parsed
        if "test_cases_run" in parsed:
            assert int(parsed.get("test_cases_run", 0)) >= 1, parsed
        if "test_cases_failed" in parsed:
            assert int(parsed.get("test_cases_failed", 0)) == 0, parsed
        if "test_cases_passed" in parsed:
            assert int(parsed.get("test_cases_passed", 0)) >= 1, parsed
        if "results" in parsed:
            for item in parsed.get("results", []):
                assert not item.get("errors"), parsed
    finally:
        browser.quit()
