...
try:
    ...
    try:
        ...
        raise RuntimeError("publish-before-pin guard requires tomllib or tomli") from exc
...
def _poetry_requirement(name: str, value):
    if not is_internal(name):
        return None
    if isinstance(value, str):
        ...
        spec = ""
    return _canon(name), _clean_spec(spec)


def pins_from_pyproject(text: str):
    """Parse dependency declarations structurally and fail closed on invalid TOML."""
    ...
    for dep in deps:
        ...
        if m and is_internal(m.group(1)):
    ...
    for table in tables:
        for name, value in table.items():
            pin = _poetry_requirement(str(name), value)
            if pin:
                out.append(pin)
    return out
