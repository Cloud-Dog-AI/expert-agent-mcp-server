...
async def validate_postgres():
    ...
    try:
        load_config.cache_clear()
        # DSN comes from config/env only (no committed connection string with a password).
        database_uri = get_config("vector_stores_config.pgvector._TEST_.database_uri")
