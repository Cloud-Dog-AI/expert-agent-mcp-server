...
async def setup_postgres():
    ...
    try:
        ...
        # Root password comes from config/env only (no committed literal).
        root_password = get_config("vector_stores_config.pgvector._DEFAULT_.root_password")
...
async def setup_mariadb():
    ...
    try:
        ...
        # Root password comes from config/env only (no committed literal).
        root_password = get_config("database.mariadb.root_password")
        ...
        try:
            ...
            # Application-user password comes from config/env only (no committed literal).
